...
class Config:
    ...
    def zhipu_api_key(self) -> Optional[str]:
        """Zhipu API Key"""
        return self._get("ZHIPU_API_KEY")

    @property
    def zhipu_base_url(self) -> str:
        """Zhipu OpenAI-compatible Base URL"""
        return self._get("ZHIPU_BASE_URL", "https://open.bigmodel.cn/api/paas/v4/")

    @property
    def zhipu_model_name(self) -> str:
        """Zhipu Model Name"""
        return self._get("ZHIPU_MODEL_NAME", "glm-4.5-air")
    ...
    def ollama_base_url(self) -> str:
        ...
        return self._get("OLLAMA_BASE_URL", "http://127.0.0.1:11434/v1")

    @property
    def local_hf_model_name(self) -> str:
        """Display name for a locally downloaded Hugging Face model."""
        return self._get("LOCAL_HF_MODEL_NAME", "Qwen3-0.6B-Base")

    @property
    def local_hf_model_path(self) -> str:
        """Absolute path to the locally downloaded Hugging Face model."""
        raw_path = self._get("LOCAL_HF_MODEL_PATH", "models/Qwen3-0.6B-Base")
        return str(self._resolve_path(raw_path))

    @property
    def local_hf_device(self) -> str:
        """Preferred device for local HF inference: auto/cuda/cpu."""
        return (self._get("LOCAL_HF_DEVICE", "auto") or "auto").lower()

    @property
    def local_hf_dtype(self) -> str:
        """Preferred dtype for local HF inference: auto/float16/bfloat16/float32."""
        return (self._get("LOCAL_HF_DTYPE", "auto") or "auto").lower()

    @property
    def local_hf_quantization(self) -> str:
        """Preferred quantization mode for local HF inference: none/4bit/8bit."""
        return (self._get("LOCAL_HF_QUANTIZATION", "4bit") or "4bit").lower()

    @property
    def local_hf_quant_type(self) -> str:
        """4-bit quantization data type for bitsandbytes, e.g. nf4/fp4."""
        return (self._get("LOCAL_HF_QUANT_TYPE", "nf4") or "nf4").lower()

    @property
    def local_hf_quant_compute_dtype(self) -> str:
        """Compute dtype used by bitsandbytes quantized inference."""
        return (self._get("LOCAL_HF_QUANT_COMPUTE_DTYPE", "float16") or "float16").lower()

    @property
    def local_hf_use_double_quant(self) -> bool:
        """Whether bitsandbytes should enable nested quantization for 4-bit loads."""
        return (self._get("LOCAL_HF_USE_DOUBLE_QUANT", "true") or "true").lower() == "true"

    @property
    def local_hf_trust_remote_code(self) -> bool:
        """Whether transformers may trust remote code for local model loading."""
        return (self._get("LOCAL_HF_TRUST_REMOTE_CODE", "false") or "false").lower() == "true"

    # ==================== 模型选择 ====================

    @property
    def model_provider(self) -> str:
        """模型提供商: dashscope, openai, ollama, local_hf"""
    ...
    def validate(self) -> tuple[bool, str]:
        ...
        if self.model_provider == "dashscope":
            ...
            if not self.zhipu_api_key:
                return False, "ZHIPU_API_KEY is required"
            ...
            # Ollama 不需要 API Key
            pass
        elif self.model_provider == "local_hf":
            model_path = Path(self.local_hf_model_path)
            if not model_path.exists():
                return False, f"LOCAL_HF_MODEL_PATH 不存在: {model_path}"
            if self.local_hf_quantization not in {"none", "off", "false", "", "4bit", "8bit"}:
                return False, "LOCAL_HF_QUANTIZATION 仅支持 none/4bit/8bit"
        else:
    ...
    def print_config(self):
        ...
        if self.model_provider == "dashscope":
            ...
            print(f"Zhipu API Key: {masked_key}")
            print(f"Zhipu Base URL: {self.zhipu_base_url}")
            print(f"Zhipu Model: {self.zhipu_model_name}")
        elif self.model_provider == "openai":
            ...
            print(f"Local HF Model: {self.local_hf_model_name}")
            print(f"Local HF Path: {self.local_hf_model_path}")
            print(f"Local HF Device: {self.local_hf_device}")
            print(f"Local HF Dtype: {self.local_hf_dtype}")
            print(f"Local HF Quantization: {self.local_hf_quantization}")
            print(f"Local HF Quant Type: {self.local_hf_quant_type}")
            print(f"Local HF Quant Compute Dtype: {self.local_hf_quant_compute_dtype}")
