  ...
  return (
    <div className="game-content-wrap">
      ...
      <div className="desk-content mar-t10">
        ...
        {
          (gameDetail.speechRecords || []).length > 0 ? (
            <div className="speech-list">
              {
                (gameDetail.speechRecords || []).map(item => {
                  return (
                    <div key={item._id} className="speech-item">
                      <span className="speech-name">{item.from ? (item.from.position + '号 ' + item.from.name) : '玩家'}：</span>
                      <span>{item.text}</span>
                    </div>
                  )
                })
          ...
          <div className="voice-action-wrap mar-t10 FBV FBAC">
            {
              gameDetail.speechTurn && gameDetail.speechTurn.currentSpeaker ? (
                <div className="voice-turn-tip">
                  {'当前发言：' + gameDetail.speechTurn.currentSpeaker.position + '号 ' + gameDetail.speechTurn.currentSpeaker.name}
                </div>
              ) : null
            }
            <Button
              className={voiceRecording ? 'btn-delete voice-btn' : 'btn-primary voice-btn'}
              loading={voiceSubmitting}
              disabled={
                !gameDetail.speechTurn ||
                !gameDetail.speechTurn.currentSpeaker ||
                gameDetail.speechTurn.currentSpeaker.username !== currentRole.username
              }
              onClick={()=>{
                voiceRecording ? onVoiceStop() : onVoiceStart()
              }}
            >
              {voiceRecording ? '结束录制' : '开始发言'}
            </Button>
          </div>
        ) : null
      }
