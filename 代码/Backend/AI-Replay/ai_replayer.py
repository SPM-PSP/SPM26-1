...
def _player_lookup(game_record: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    lookup: Dict[str, Dict[str, Any]] = {}
    for player in _compact_players(game_record):
...
def _player_display_name(player_id: Any, players: Dict[str, Dict[str, Any]]) -> str:
    key = str(player_id or "").strip()
    player = players.get(key, {})
    return str(player.get("name") or key or "未知玩家")


def _player_role_label(player_id: Any, players: Dict[str, Dict[str, Any]]) -> str:
    key = str(player_id or "").strip()
    player = players.get(key, {})
    return str(player.get("role_label") or ROLE_LABELS.get(player.get("role"), player.get("role")) or "未知身份")


def _player_position_name(player_id: Any, players: Dict[str, Dict[str, Any]]) -> str:
    key = str(player_id or "").strip()
    player = players.get(key, {})
    position = player.get("position")
    name = player.get("name") or key or "未知玩家"
    if position not in (None, ""):
        return f"{position}号（{name}）"
    return str(name)


def _fixed_vote_analysis(game_record: Dict[str, Any], raw_result: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    players = _player_lookup(game_record)
    fixed: Dict[str, Any] = {"rounds": []}
    raw_vote_analysis = raw_result.get("vote_analysis") if isinstance(raw_result, dict) else {}
    raw_rounds = raw_vote_analysis.get("rounds", []) if isinstance(raw_vote_analysis, dict) else []

    for index, rr in enumerate(game_record.get("round_records", []) or [], start=1):
        if not isinstance(rr, dict):
            continue
        vote_results = rr.get("vote_results", {}) if isinstance(rr.get("vote_results"), dict) else {}
        vote_counts = vote_results.get("vote_counts", {}) if isinstance(vote_results.get("vote_counts"), dict) else {}
        if not vote_counts:
            continue

        day = rr.get("day") or rr.get("round") or index
        round_no = rr.get("round") or day or index
        display_counts = vote_results.get("vote_counts_by_name") if isinstance(vote_results.get("vote_counts_by_name"), dict) else None
        if not display_counts:
            display_counts = {
                _player_display_name(player_id, players): count
                for player_id, count in vote_counts.items()
            }

        voted_out = vote_results.get("voted_out")
        voted_out_id = None
        if isinstance(voted_out, dict):
            voted_out_id = voted_out.get("username") or voted_out.get("player_id")
        elif voted_out:
            voted_out_id = voted_out
        if not voted_out_id:
            voted_out_name = str(vote_results.get("voted_out_name") or "")
            for player_id, player in players.items():
                if player_id in voted_out_name or str(player.get("name") or "") in voted_out_name:
                    voted_out_id = player_id
                    break

        if vote_results.get("voted_out_display"):
            voted_out_display = str(vote_results.get("voted_out_display"))
            voted_out_round_display = _player_position_name(voted_out_id, players) if voted_out_id else str(vote_results.get("voted_out_name") or voted_out_display)
        elif voted_out_id:
            voted_out_display = f"{_player_display_name(voted_out_id, players)}（{_player_role_label(voted_out_id, players)}）"
            voted_out_round_display = _player_position_name(voted_out_id, players)
        else:
            voted_out_display = "无玩家出局"
            voted_out_round_display = "无玩家出局"

        raw_round = raw_rounds[index - 1] if isinstance(raw_rounds, list) and index - 1 < len(raw_rounds) and isinstance(raw_rounds[index - 1], dict) else {}
        analysis_text = str(raw_round.get("analysis") or "").strip()
        if not analysis_text:
            top_items = sorted(display_counts.items(), key=lambda item: int(item[1] or 0), reverse=True)
            top_text = "、".join([f"{name}{count}票" for name, count in top_items[:3]])
            if voted_out_id:
                analysis_text = f"第{day}天投票中，{voted_out_display}被投出局。票型为：{top_text}。"
            else:
                analysis_text = f"第{day}天投票中没有玩家出局。票型为：{top_text}。"

        fixed[f"day{day}"] = {
            "vote_counts": display_counts,
            "voted_out": voted_out_display,
        ...
        fixed["rounds"].append(
            {
                ...
                "voted_out": voted_out_round_display,
            }
        )
...
def _fallback_vote_analysis(raw_result: Dict[str, Any], game_record: Dict[str, Any]) -> Any:
    fixed_vote_analysis = _fixed_vote_analysis(game_record, raw_result)
    if fixed_vote_analysis.get("rounds"):
        return fixed_vote_analysis
...
def _normalize_analysis_result(raw_result: Dict[str, Any], game_record: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    normalized = dict(raw_result)
    fixed_vote_analysis = _fallback_vote_analysis(raw_result, game_record or {})
    if isinstance(fixed_vote_analysis, dict):
        for key, value in fixed_vote_analysis.items():
            if re.match(r"^day\d+$", str(key)):
                normalized[key] = value
        normalized["vote_analysis"] = {"rounds": fixed_vote_analysis.get("rounds", [])}
    else:
        normalized["vote_analysis"] = fixed_vote_analysis
    normalized["recommendation_layers"] = recommendation_layers
    normalized["strategy_recommendations"] = _flatten_recommendation_layers(recommendation_layers)
    normalized["action_items_structured"] = action_items_structured
    normalized["action_items"] = [
...
def _heuristic_analysis(game_record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    fixed_vote_analysis = _fixed_vote_analysis(game_record, {})

    analysis = {
        "vote_analysis": {"rounds": fixed_vote_analysis.get("rounds", [])},
...
def analyze_game_record(
    ...
    if ai_available:
        try:
            ...
            ai_timeout = float(ai_config.get("timeout") or os.getenv("AI_REPLAY_MODEL_TIMEOUT", "90"))
            client = OpenAI(api_key=api_key, base_url=baseurl, timeout=ai_timeout, max_retries=0)
            ...
            system = (
                ...
                "票型格式必须固定：每个投票日必须输出 day1、day2 这种顶层对象，每个 dayN 只能包含 vote_counts、voted_out、analysis 三个字段；"
                "vote_analysis 必须是对象，且只能包含 rounds 数组；rounds 每项只能包含 round、vote_counts、voted_out 三个字段。"
                "dayN.vote_counts 使用玩家显示名称作 key；vote_analysis.rounds[].vote_counts 使用玩家原始 player_id/username 作 key。"
