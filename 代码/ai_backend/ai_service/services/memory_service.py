...
class MemoryService:
    ...
    def append_event(self, request: AppendMemoryEventRequest) -> AgentMemoryState:
        ...
        state.memory_window.append(request.event)
        if len(state.memory_window) > self.max_window:
            state.memory_window = state.memory_window[-self.max_window :]

        # 维护简单关系图：记录“最近互动”
        if request.event.speaker and request.event.speaker != request.ai_id:
            state.relations[request.event.speaker] = "recent_interaction"
