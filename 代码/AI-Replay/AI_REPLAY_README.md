...
> AI 复盘模块采用 Node 后端加 Python AI 服务的拆分架构。Node 后端负责从 MySQL 中读取真实对局数据，包括玩家身份、发言、投票、技能动作和胜负结果，并整理成结构化 `game_record`。Python 服务负责调用大模型生成复盘报告，输出 JSON 和 TXT 两种格式。最终通过 `replay_index.json` 按 `gameId` 建立索引，前端可以快速查询和展示。

如果老师问为什么要拆成两个服务：

> 因为游戏主流程要求稳定和实时，而 AI 复盘可能比较耗时。把 AI 分析拆成独立 Python 服务，可以降低耦合。即使 AI 服务失败，也不会影响游戏正常进行，后续还可以重新生成复盘。

如果老师问 AI 复盘是否准确：

> AI 的基础事实来自数据库中的结构化记录，比如角色、投票、技能、死亡和胜负，这些不是 AI 猜的。AI 主要负责总结和分析，所以复盘结果有可靠的数据基础。

如果老师问复盘怎么避免重复生成：

> 后端通过 `replay_index.json` 缓存每个 `gameId` 的复盘文件。如果已有文件存在，就直接复用；只有强制刷新或文件不存在时才重新调用 AI。
...
1. 增加历史对局列表，让用户不用手动切换 `gameId`。
2. 增加复盘生成进度展示，避免用户以为页面卡住。
3. 把复盘任务改成后台队列，支持异步任务状态查询。
4. 为阶段结算、胜负判断、复盘生成补充自动化测试。
5. 在复盘页面增加时间线、票型图、玩家个人视角分析。
