...
本章旨在详细规范系统如何通过技术手段满足《需求分析文档》中的非功能性需求。设计核心围绕“高并发实时状态机”与“异步 AI 推理流水线”展开。

### 7.1 异步任务流转设计
系统利用 **Fastify** 的异步事件驱动特性，在架构逻辑上将“即时指令”与“耗时计算”完全解耦，确保游戏逻辑主线程的绝对优先权。
...
sequenceDiagram
    ...
    participant A as AI/语音服务层 (Python)
...
为避免频繁读写 MySQL 导致 IO 等待，系统对在线对局的活跃数据采用 **Memory-First** 策略：
* **存储结构**：
    * `Room:State:{RoomID}`: 存储 Hash 结构，包含当前阶段（昼夜）、发言索引、存活列表。
    * `Room:Timer:{RoomID}`: 存储 String 结构，设置过期时间实现倒计时自动切换逻辑。
* **同步时机**：仅在“天黑”、“天亮”、“放逐”等核心逻辑节点（涉及胜负判定）执行 `MySQL.Transaction` 批量持久化。

#### 7.2.2 WebSocket 流量削峰策略（针对 SRS-PERF-06）
* **增量同步 (Incremental Sync)**：服务端禁止下发整个房间对象，仅发送 `diff` 报文。例如：`{"type":"VOTE","data":{"voter":1,"target":5}}`，报文体积控制在 1KB 以内。
* **频率限制 (Throttling)**：对于非关键性状态（如玩家在输入框输入状态），设置 500ms 的节流合并，减少服务器推送压力。
...
#### 7.3.1 AI 推理异步流转逻辑
为防止 LLM 推理（通常耗时 1-3s）阻塞 Fastify 主线程，系统通过异步回调机制实现性能闭环：
...
* **语音分片流式预处理**：针对 **SRS-PERF-05**，系统在语音服务层采用“边录边转”技术。前端利用 `Web Audio API` 将音频进行分片（Chunk）上传，使 STT（语音转文字）服务能提前进入计算状态，显著缩短发言结束后的转写等待时间。
* **任务并发限流 (Throttling)**：系统设置 `MAX_AI_CONCURRENCY` 阈值。当并发推理请求激增时，调度器会自动执行排队策略或切换至轻量级模型（如裁剪版的 Whisper），以牺牲极小部分的准确率换取整体系统的响应时效。

### 7.4 性能评估与监控矩阵

| 指标 ID | 性能维度 | 技术实现策略 | 监控方案 |
| :--- | :--- | :--- | :--- |
| **P-01** | REST 接口响应 | Fastify 路由加速 + 数据库索引优化 | 开启 Fastify 性能采集日志 |
| **P-02** | 状态同步延迟 | WebSocket 增量推送 + Redis 缓存 | 埋点计算 `Server_Push_Time` |
| **P-03** | 数据库并发 | Sequelize 连接池（10~50 动态预热） | MySQL `Slow_Query_Log` 监控 |
