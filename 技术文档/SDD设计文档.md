...
4. 当前实现情况：
1. `[已实现]` 登录与 JWT。
2. `[已实现]` 路由层 `/auth` URL 前缀鉴权。
3. `[可推断]` 预留了 URL 权限缓存校验，但缓存加载链路未见完整实现。
5. 可改进点：
1. 密码目前为 SHA-256 直哈希，建议改为 `bcrypt/argon2`。
2. URL 权限缓存键 `page_url_permission` 未见填充逻辑，权限链路不完整。
...
3. 当前实现情况：
1. `[已实现]` 房间生命周期核心操作。
2. `[已实现]` 观战名单维护（`room.ob`）。
3. 可改进点：
1. 部分写操作使用 GET（REST 语义不严格）。
...
### 5.4 角色分配模块
1. 模块职责：
1. 按人数板子分配角色，生成局内玩家与视野矩阵。
2. 主要输入输出：
1. 输入：房间座位、模式映射、技能映射。
2. 输出：`player`、`vision`、`game` 初始状态。
3. 核心实现：
...
3. 当前实现情况：
1. `[已实现]` 6-12人标准板子。
2. `[已实现]` 狼队互认视野、预言家初始未知视野。
3. 可改进点：
1. 开局涉及多表连续写入，缺少事务回滚设计。

### 5.5 白天发言/投票模块
1. 模块职责：
1. 发言顺序生成、投票记录、平票处理、放逐结算。
2. 主要输入输出：
1. 输入：投票动作、存活玩家集。
2. 输出：票型记录、放逐目标、PK标签。
3. 核心实现：
...
2. 主要输入输出：
1. 输入：技能动作。
2. 输出：`action`、`record`、`gameTag`、玩家状态变更。
3. 核心实现：
...
2. 主要输入输出：
1. 输入：玩家存活分布、胜利条件配置。
2. 输出：`game.status=2`、`winner`、结束记录、`gameOver` 推送。
3. 核心实现：
...
3. 当前实现情况：
1. `[已实现]` 事件字符串协议：`refreshRoom/refreshGame/stageChange/gameStart/gameOver/reStart`。
2. `[已实现]` 倒计时 JSON 消息 `{refreshGame:false,time:n}`。
3. 可改进点：
...
3. 可改进点：
1. 表间外键与事务边界未系统化。
2. 建议补充归档策略与大表治理（`record/action/tag`）。
